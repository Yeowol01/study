...
#pragma region 인라인 함수
	...
	// 인라인 함수는 함수를 호출하는 과정이 없으므로 처리 속도가 빠르지만, 인라인 함수를 많이 사용하게 되면 함수의 코드가 복사되기 때문에 실행 파일의 크기가 커지게 됩니다.
	//Information();
...
#pragma region 정수의 승격
	...
	// 일반적으로 CPU가 처리하기에 가장 적합한 크기의 정수 자료형은 int 이므로, int형 보다 작은 자료형은 int 자료형으로 변환하는 것입니다.
